...
# Use learnfm's dx7 to render a dx7 note from MSFA
def dx7_render_patch(patch_data, midinote, velocity, samples, keyup_sample):
    s = dx7.render_patchdata(patch_data, midinote, velocity, samples, keyup_sample)
    return np.array(s)/32767.0
...
def play_patch(patch_number, midinote=50, length_s = 2, keyup_s = 1, modified_ralph=None):
    ...
    # Render Ralph
    if(modified_ralph is not None):
        them_samples = dx7_render_patch(bytes(modified_ralph), midinote, 90, int(length_s*alles.SAMPLE_RATE),int(keyup_s*alles.SAMPLE_RATE))
    else:
