...
class AMYPatch:
    ...
    def send_to_AMY(self):
        ...
        amp_lfo_amp = dx7level_to_linear(self.lfo_ampmoddepth)
        ...
        print("osc 7 amp lfo wave %d freq %f amp %f" % (
            self.lfo_waveform, self.lfo_freq, amp_lfo_amp))
        alles.send(osc=7, wave=self.lfo_waveform, freq=self.lfo_freq,
                   amp=amp_lfo_amp)

        # and the pitch one
        print("osc 8 pitch lfo wave %d freq %f amp %f" % (
            self.lfo_waveform, self.lfo_freq, dx7level_to_linear(self.lfo_pitchmoddepth)))
        alles.send(osc=8, wave=self.lfo_waveform, freq=self.lfo_freq,
                   amp=dx7level_to_linear(self.lfo_pitchmoddepth))
...
def generate_fm_header(patch_nums):
    ...
    # For now, report first lfo-enabled osc as target.
    lfo_target = -1
    for osc_num, osc in enumerate(p.oscs):
        if osc.ampmodsens > 0:
            lfo_target = osc_num
            break
    ...
    for idx, p in enumerate(all_patches):
        out.write("\t{ %d, %f, {%f, %f, %f, %f, %f}, {%d, %d, %d, %d, %d}, %f, %d, %f, %d, {\n" % (
            p.algo, p.feedback,
            ...
            p.lfo_freq, p.lfo_waveform, p.lfo_ampmoddepth, lfo_target))
        for i, osc in enumerate(p.oscs):
            detune = 0  # No longer separately recorded.
            if osc.freq_is_ratio:
                ratio = osc.frequency
                frequency = -1
            else:
            ...
            out.write("\t\t\t{%f, %f, %f, {%f, %f, %f, %f, %f}, {%d, %d, %d, %d, %d}, %f}, /* op %d */\n" % (
