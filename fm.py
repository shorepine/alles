...
def setup_patch(p):
	...
	for i,op in enumerate(p["ops"]):
		...
		if(freq>=0):
			amy.send(osc=i, freq=freq, ratio=freq_ratio,bp0_target=amy.TARGET_AMP+amy.TARGET_LINEAR,bp0=op["bp_opamp"], bp1=pitchenv, bp1_target=amy.TARGET_FREQ+amy.TARGET_LINEAR, amp=amp, detune=op["detunehz"])
		else:
			amy.send(osc=i, freq=freq, ratio=freq_ratio,bp0_target=amy.TARGET_AMP+amy.TARGET_LINEAR,bp0=op["bp_opamp"], amp=amp, detune=op["detunehz"])
...
def header_patch(p):
	...
	for i,op in enumerate(p["ops"]):
		...
		if(op.get("fixedhz",None) is not None):
			...
			freq_ratio = op["ratio"]
		o_data = [freq, freq_ratio, op["opamp"], op["bp_opamp"], p["bp_pitch"], op["detunehz"]]
		os.append(o_data)
	...
	feedback = p["feedback"]/28.0
	return (p["name"], p["algo"], feedback, p["bp_pitch"], lfo_freq, lfo_wave, amp_lfo_amp, freq_lfo_amp, os)
...
def decode_patch(p):
	...
	patch["bp_pitch"] = eg_to_bp_pitch([x for x in p[c:c+4]], [x for x in p[c+4:c+8]])
	c = c + 8
	patch["algo"] = p[c]
	c = c + 1
	patch["feedback"] = p[c]
