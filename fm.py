...
# wave = fm.synth_dx7_patch(dx7_patch)
# plt.plot(wave)
...
def calc_loglin_eg_env(breakpoints, keyup_time=0.5, frame_rate=1000, do_exp=True):
    ...
    for segment, breakpoint in enumerate(breakpoints):
        # Work in DX7 levels.
        target_level = linear_to_dx7level(breakpoint[1])
...
def dx7_op_waveform(op, f0=440, mod=None, sr=44100, duration=1.0, keyup_time=0.5, feedback=0):
    ...
    if op.ratiotuning:
        frequency = f0 * coarse_fine_ratio(op.freq_coarse, op.freq_fine, op.freq_detune)
    else:
        frequency = coarse_fine_fixed_hz(op.freq_coarse, op.freq_fine, op.freq_detune)
    if op.rates is not None:
        bps = calc_loglin_eg_breakpoints(op.rates, op.levels)
        env = calc_loglin_eg_env(bps, frame_rate=sr, do_exp=True, keyup_time=keyup_time)
    else:
        env = np.ones(n_samps)
    if len(env) < n_samps:
        env = np.concatenate([env, dx7level_to_linear(op.levels[-1]) * np.ones(n_samps - len(env))])
    else:
        env = env[:n_samps]
    env *= 2 * dx7level_to_linear(op.opamp)    
